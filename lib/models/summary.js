...
/**
    ...
    @return {Article}
*/
Summary.prototype.getNextPageFirstArticle = function(current) {
    var level = is.string(current)? current : current.getLevel();
    var path = is.string(current)? current : current.getPath();
    var seenCurrent = false;

    return this.getArticle(function(article) {
        if (seenCurrent && article.getPath() != path) 
            return true;
...
/**
    ...
    @return {Article}
*/
Summary.prototype.getPrevPageLastArticle = function(current) {
    var level = is.string(current)? current : current.getLevel();
    var path = is.string(current)? current : current.getPath();
    var prev = undefined;
