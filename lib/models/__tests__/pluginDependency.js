    ...
    describe('createFromString', function() {
        ...
        describe('GIT Version', function() {
            it('must handle HTTPS urls', function() {
                var plugin = PluginDependency.createFromString('hello@git+https://github.com/gitbook-ng/plugin-ga.git');
                expect(plugin.getName()).toBe('hello');
                expect(plugin.getVersion()).toBe('git+https://github.com/gitbook-ng/plugin-ga.git');
            });

            it('must handle SSH urls', function() {
                var plugin = PluginDependency.createFromString('hello@git+ssh://samy@github.com/gitbook-ng/plugin-ga.git');
                expect(plugin.getName()).toBe('hello');
                expect(plugin.getVersion()).toBe('git+ssh://samy@github.com/gitbook-ng/plugin-ga.git');
            });
        });
        ...
        describe('listFromArray', function() {
            it('must create an array from a list of plugin dependencies', function() {
                var arr = Immutable.fromJS([
                    ...
                    {
                        'name': 'plugin-ga',
                        'version': 'git+ssh://samy@github.com/gitbook-ng/plugin-ga.git'
                    }
                ]);
                ...
                expect(list.last().getVersion()).toBe('git+ssh://samy@github.com/gitbook-ng/plugin-ga.git');
            });
        });
    });
});
